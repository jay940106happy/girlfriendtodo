...
onMounted(async () => {
  ...
  deferredMemoriesTimerId = setTimeout(startMemoriesFetch, MEMORY_FETCH_DEFER_MS)
})

onUnmounted(() => {
  if (celebrationTimerId) clearTimeout(celebrationTimerId)
  if (deferredMemoriesTimerId) clearTimeout(deferredMemoriesTimerId)
})

watch(activePage, (nextPage) => {
  if (nextPage === 'memories' || nextPage === 'calendar') startMemoriesFetch()
...
<template>
  <div class="app-shell" :class="{ 'app-shell--celebration': celebrationActive }">
    ...
    <button class="floating-add" type="button" :aria-label="`新增${pageTitle}`" @click="activePage === 'todos' ? openTodoComposer() : openMemoryComposer()">+</button>
